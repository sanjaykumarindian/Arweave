...
The browser entry point binds `window.fetch` to `window` before the Wayfinder SDK module graph loads. This avoids the browser `Illegal invocation` error that occurs when a native `Window.fetch` function is called without its required receiver.

The app also no longer initializes an unused `useWayfinderUrl({ txId: "" })` hook or makes an unnecessary direct `arweave.net` HEAD request.
